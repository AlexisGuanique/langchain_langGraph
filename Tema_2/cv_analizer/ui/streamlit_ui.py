...
def main():
    ...
    if not api_key:
        ...
        st.info("""
        📝 **La API Key de OpenAI debe estar configurada como variable de entorno.**
        
        Configura la variable de entorno `OPENAI_API_KEY` en el contenedor Docker.
        
        Ejemplo:
        ```bash
        docker run -d -p 8501:8501 -e OPENAI_API_KEY=tu_api_key_aqui --name cv-analizer cv-analizer
        ```
